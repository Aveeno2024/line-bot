...
const CITIES = [
  ...
  { code: "2", name: "新北市", displayName: "新北市", stationId: "46692" }, // 用台北站
  ...
  { code: "A", name: "雲林縣", displayName: "雲林縣", stationId: "46748" }, // 用嘉義站
  { code: "B", name: "嘉義縣", displayName: "嘉義縣", stationId: "46748" },
  { code: "C", name: "彰化縣", displayName: "彰化縣", stationId: "46749" }, // 用台中站
  { code: "D", name: "臺中市", displayName: "台中市", stationId: "46749" },
  { code: "E", name: "南投縣", displayName: "南投縣", stationId: "46765" },
  { code: "F", name: "苗栗縣", displayName: "苗栗縣", stationId: "46757" }, // 用新竹站
...
async function getRealWeather(city) {
  try {
    const url = `https://opendata.cwa.gov.tw/api/v1/rest/datastore/O-A0001-001?Authorization=${CWA_API_KEY}&format=JSON&StationId=${city.stationId}`;
    
    console.log(`🌤️ 獲取 ${city.displayName} (站號:${city.stationId})`);
    ...
    if (data.success === "true" && data.records?.Station?.[0]) {
      ...
      if (weather && weather.AirTemperature && weather.RelativeHumidity) {
    ...
    console.log(`⚠️ ${city.displayName} 無有效觀測資料`);
    return null;
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`✅ API 金鑰已設定: ${CWA_API_KEY ? '是' : '否'}`);
